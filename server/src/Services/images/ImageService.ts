...
import { SaveRepository } from "../../Database/repositories/saves/SaveRepository";
...
export class ImageService implements IImageService {
  ...
  async getImages(page: number, limit: number, category?: string, search?: string, currentUserId?: number): Promise<any> {
    ...
    // isLiked and isSaved are already set in repository if currentUserId is provided
  ...
  async getPopularImages(page: number, limit: number, sortType: string = 'likes', currentUserId?: number): Promise<any> {
    const { images, total } = await this.imageRepository.getPopular(page, limit, sortType);
    ...
    if (currentUserId) {
      for (const image of images) {
        ...
        image.isSaved = await this.saveRepository.isSavedByUser(image.id, currentUserId);
      }
    }
  ...
  async unlikeImage(imageId: number, userId: number): Promise<boolean> {
    // Remove like
    return await this.likeRepository.delete(userId, imageId);
  }
  ...
  async saveImage(imageId: number, userId: number, collectionId: number): Promise<boolean> {
    ...
    const existingSave = await this.saveRepository.findByUserAndImage(userId, imageId);
    if (existingSave) {
      // Update collection if different
      if (existingSave.collectionId !== collectionId) {
        await this.saveRepository.delete(userId, imageId);
        return await this.saveRepository.create(userId, imageId, collectionId);
      }
      return true;
    }
  ...
  async unsaveImage(imageId: number, userId: number): Promise<boolean> {
    // Remove from saved
    return await this.saveRepository.delete(userId, imageId);
  }
